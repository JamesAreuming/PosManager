/*
 * Filename	: pushMarketingManage.js
 * Function	:
 * Comment 	:
 * History	: 
 *
...
define([
  ...
  'text!templates/marketing/pushMarketingManage.html',
  'text!templates/marketing/modal/pushMarketing_modal.html',
  'text!templates/common/modal_form_alert.html',

  'bootstrap-daterangepicker'

], function (Backbone, Mustache, Common, i18n, template, pushMarketingModal, modalAlert) {
  'use strict';
  ...
  var _def_form_mustache =
	{
	  'id' : '{{id}}',
	  'brandId' : '{{brandId}}',
	  'pushMarketingNm': '{{pushMarketingNm}}',
	  'term' : '{{term}}',
	  'begin' : '{{begin}}',
	  'expire' : '{{expire}}',
	  'discount' : '{{discount}}',
  ...
  var targetUrl = './model/marketing/PushMarketing';
  ...
  var ContentView = Backbone.View.extend({
    el: $('#main-wrapper'),
    events : {
    	'click #new-pushMarketing' : 'modal_init',
    	'click #pushMarketing_table button.pushMarketing-detail' : 'load_detail'/*,
    ...
    modal_init : function(){
    	...
    	if(_data.brandId != undefined && _data.brandId != ''){
			...
			$('#pushMarketing-modal').html(Mustache.to_html(pushMarketingModal, data));
			view.form_init('POST', $('#pushMarketing-modal'));

    	} else {
    		alert(i18n.BO4002);
    		$('#pushMarketing-modal').html('');
    		$('#pushMarketing-modal').modal('toggle');
    	}
    },
    datatablesInit : function(){
    	main_tables = $('#pushMarketing_table').DataTable({
	  		"serverSide": true,
	    	"language": {
		    ...
		    "columnDefs":  [ {
		        "render": function ( data, type, row ) {
		            return '<button class="btn btn-primary btn-sm pushMarketing-detail" data-toggle="modal" data-target=".bs-pushMarketing-modal-lg" target-code="'+data+'" >상세내역</button>';
		        },
		        "targets": 7
		    } ],
		    "pageLength": 10,
    ...
    load_detail : function(event){
    	...
    	$.ajax({
    		...
    		success : function(response){
    			if(response.success){
    				var data = view.loadBaseCode();
    				var _template = Mustache.to_html(pushMarketingModal, $.extend(data, _def_form_mustache));

    				$('#pushMarketing-modal').html(Mustache.to_html(_template, view.buildOptions(response.data)));
    				view.form_init('PUT', $('#pushMarketing-modal'));
