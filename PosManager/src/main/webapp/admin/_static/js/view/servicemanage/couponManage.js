...
define([
  ...
  'text!templates/servicemanage/couponManage.html',
  'i18n!templates/globals/lang/nls/language',
  
  'text!templates/servicemanage/modal/coupon_modal.html',
  
  'text!templates/sysmanage/modal/modal_form_alert.html',
  
  'bootstrap-daterangepicker'
  
], function (Backbone, Mustache, template, i18n, couponModal, modalAlert) {
  'use strict';
  ...
  var ContentView = Backbone.View.extend({
    el: $('#main-wrapper'),
    events : {
    	'click #new-coupon' : 'modal_init',
    	'click #coupon_table button.coupon-detail' : 'load_detail'/*,
    ...
    modal_init : function(){
    	...
    	if(_data.brandId != undefined && _data.brandId != ''){
			...
			$('#coupon-modal').html(Mustache.to_html(couponModal, data));
    	...
    	main_tables = $('#coupon_table').DataTable({
		    ...
		    "columnDefs":  [ {
		        "render": function ( data, type, row ) {
		            return '<button class="btn btn-primary btn-sm coupon-detail" data-toggle="modal" data-target=".bs-coupon-modal-lg" target-code="'+data+'" >상세내역</button>'; 
		        },
		        "targets": 7		    
		    } ],
		    "pageLength": 10,
    ...
    load_detail : function(event){
    	...
    	$.ajax({
    		...
    		success : function(response){
    			if(response.success){
    				var data = view.loadBaseCode();
    				var _template = Mustache.to_html(couponModal, $.extend(data, _def_form_mustache));
    				
    				$('#coupon-modal').html(Mustache.to_html(_template, view.buildOptions(response.data)));
