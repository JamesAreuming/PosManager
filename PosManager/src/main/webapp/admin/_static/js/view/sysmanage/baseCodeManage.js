...
define([
  ...
  'text!templates/sysmanage/baseCodeManage.html',
  'i18n!templates/globals/lang/nls/language',
  
  'text!templates/sysmanage/modal/basecode_modal_main.html',
  
  'text!templates/sysmanage/modal/basecode_modal_sub.html',
  
  'text!templates/sysmanage/modal/modal_form_alert.html'
], function (Backbone, Mustache, template, i18n, main_modal, sub_modal, modalAlert) {
  'use strict';
  ...
  var ContentView = Backbone.View.extend({
      ...
      //Index.initCharts(); // init index page's custom scripts
	    ...
	    sub_tables = $('#sub_code').DataTable({
		    ...
		    "columnDefs":  [ {
		        ...
		        "render": function ( data, type, row ) {
		            return '<button class="btn btn-primary btn-sm corp-detail" data-toggle="modal" data-target=".bs-sub-modal-lg" target-code="'+data+'" target-id="'+row.id+'">상세내역</button>'; 
		        },
		        "targets": 6
		    } ],
		    "pageLength": 10,
    ...
    modal_init : function(event){
    	$('#main-modal').html(Mustache.to_html(main_modal, {}));
    	this.form_init('POST', $('#main-modal'), main_tables);
    },
    sub_init : function(){
    	if(main_tables.$('tr.selected').length == 1){
        	$('#sub-modal').html(Mustache.to_html(sub_modal, { mainCd : main_tables.row(main_tables.$('tr.selected')).data().mainCd, mainId : main_tables.row(main_tables.$('tr.selected')).data().id}));
