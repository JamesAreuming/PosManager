...
define([
  ...
  'text!templates/items/couseItemManage.html',
  'text!templates/items/modal/couseItem_modal.html',
  'text!templates/common/modal_form_alert.html',

  'bootstrap-daterangepicker'

], function (Backbone, Mustache, Common, i18n, template, couseItemModal, modalAlert) {
  'use strict';
  ...
  var targetUrl = './model/items/CouseItem';
  ...
  var ContentView = Backbone.View.extend({
    el: $('#main-wrapper'),
    events : {
    	'click #new-couseItem' : 'modal_init',
    	'click #couseItem_table button.couseItem-detail' : 'load_detail'/*,
    ...
    modal_init : function(){
    	...
    	if(_data.brandId != undefined && _data.brandId != ''){
			...
			$('#couseItem-modal').html(Mustache.to_html(couseItemModal, data));
			view.form_init('POST', $('#couseItem-modal'));

    	} else {
    		alert(i18n.BO4002);
    		$('#couseItem-modal').html('');
    		$('#couseItem-modal').modal('toggle');
    	}
    },
    datatablesInit : function(){
    	main_tables = $('#couseItem_table').DataTable({
	  		"serverSide": true,
	    	"language": {
		    ...
		    "columnDefs":  [ {
		        "render": function ( data, type, row ) {
		            return '<button class="btn btn-primary btn-sm couseItem-detail" data-toggle="modal" data-target=".bs-couseItem-modal-lg" target-code="'+data+'" >상세내역</button>';
		        },
		        "targets": 7
		    } ],
		    "pageLength": 10,
    ...
    load_detail : function(event){
    	...
    	$.ajax({
    		...
    		success : function(response){
    			if(response.success){
    				var data = view.loadBaseCode();
    				var _template = Mustache.to_html(couseItemModal, $.extend(data, _def_form_mustache));

    				$('#couseItem-modal').html(Mustache.to_html(_template, view.buildOptions(response.data)));
    				view.form_init('PUT', $('#couseItem-modal'));
