...
define([
  ...
  'text!templates/items/setItemManage.html',
  'text!templates/items/modal/setItem_modal.html',
  'text!templates/common/modal_form_alert.html',

  'bootstrap-daterangepicker'

], function (Backbone, Mustache, Common, i18n, template, setItemModal, modalAlert) {
  'use strict';
  ...
  var targetUrl = './model/items/SetItem';
  ...
  var ContentView = Backbone.View.extend({
    el: $('#main-wrapper'),
    events : {
    	'click #new-setItem' : 'modal_init',
    	'click #setItem_table button.setItem-detail' : 'load_detail'/*,
    ...
    modal_init : function(){
    	...
    	if(_data.brandId != undefined && _data.brandId != ''){
			...
			$('#setItem-modal').html(Mustache.to_html(setItemModal, data));
			view.form_init('POST', $('#setItem-modal'));

    	} else {
    		alert(i18n.BO4002);
    		$('#setItem-modal').html('');
    		$('#setItem-modal').modal('toggle');
    	}
    },
    datatablesInit : function(){
    	main_tables = $('#setItem_table').DataTable({
	  		"serverSide": true,
	    	"language": {
		    ...
		    "columnDefs":  [ {
		        "render": function ( data, type, row ) {
		            return '<button class="btn btn-primary btn-sm setItem-detail" data-toggle="modal" data-target=".bs-setItem-modal-lg" target-code="'+data+'" >상세내역</button>';
		        },
		        "targets": 7
		    } ],
		    "pageLength": 10,
    ...
    load_detail : function(event){
    	...
    	$.ajax({
    		...
    		success : function(response){
    			if(response.success){
    				var data = view.loadBaseCode();
    				var _template = Mustache.to_html(setItemModal, $.extend(data, _def_form_mustache));

    				$('#setItem-modal').html(Mustache.to_html(_template, view.buildOptions(response.data)));
    				view.form_init('PUT', $('#setItem-modal'));
