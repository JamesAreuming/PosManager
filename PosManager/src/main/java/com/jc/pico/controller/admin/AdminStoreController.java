...
@Controller
@ResponseBody
@RequestMapping("/admin/model/store")
public class AdminStoreController {

  protected static Logger logger = LoggerFactory.getLogger(AdminStoreController.class);
...
//		}
  ...
  public Map<Object,Object> Store( 
    ...
    case GET :
    	...
    	Boolean a = !str.equals("null");
    	System.out.println("확인+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"+a); // false */
      ...
      //조건[1] - null일때 
    	  ...
    	  if ("service".equals(mode)) {
    		  ...
    		  svcStaff = svcStaffMapper.selectByExample(example); // 해당 스탭 리스트를 가져옴
    	  ...
    	  else if ("printer".equals(mode)) { // 프린터
    ...
    case POST :
      ...
      logger.debug(" mode POST : " + mode);  // mode POST : staff  -- 스탭 정보 변경   -->staff
      ...
      } else if ("staff".equals(mode)) {  // mode POST : staff 

  		Map<String, String> map = StrUtils.getStartWithParam(search, "staffId");
  		
  		if (MapUtils.isNotEmpty(map)) { //map 널값 체크
  			Iterator<String> it = map.keySet().iterator();
  			while (it.hasNext()) {
      	  ...
      	  if ("info".equals(mode) || !StringUtils.hasText(mode)) {
      		  if (customStoreMapper.getCheckStore(search) == 0) {
      			  ...
      			  if(svcStoreMapper.insertSelective(store) == 1 ? true : false){    				  
      				  ...
      				  if(brandSet != null){
      			    	  ...
      			    	  logger.debug("★★★★★★★★★★★"); 
      					    logger.debug("storeSet 1: " + JsonConvert.toJson(storeSet)); 
      			    	  logger.debug("★★★★★★★★★★★");      			    	  
      		  ...
      		  if(svcStoreMapper.insertSelective(store) == 1 ? true : false){
  				  ...
  				  if(brandSet != null){
  			    	  ...
  			    	  logger.debug("★★★★★★★★★★★"); 
  					    logger.debug("storeSet 2: " + JsonConvert.toJson(storeSet)); 
  			    	  logger.debug("★★★★★★★★★★★");  
      ...
      try {
        if ("service".equals(mode)) {
          ...
          result.put("data", printer);
      	  ...
      	  //카드결제를 위한 정보 	     
      	  boolean flag2 = false;  
      	  storeSet.setId((StrUtils.parseLong(search.get("storesetId"))));
      	  flag2 = svcStoreSetMapper.updateByPrimaryKeySelective(storeSet) == 1 ? true : false;   
      	  
      	  result.put("success", flag);
      	  result.put("storeset", flag2);
...
//			  result.put("errMsg", "");
  ...
  /**
   * Store - POS License  // ORder9 Backoffice > 업체관리 > 라이센스 > [버튼]라이센스 일괄등록
   * @param license
   * @param params
   * @param start
   * @param length
  ...
  public Map<Object,Object> PosLicense(
      ...
      logger.debug("license >> " + license); // null
      logger.debug("param >> " + params.get("data")); // null
	  
	  if (license == null && params.get("data") != null) { // 전체 라이센스 리스트 출력시 적용 X, 라이센스 일괄등록시 적용 O
	  ...
	  logger.debug("★★★★★★ MAPPING : /admin/model/store/PosLicense ★★★★★★");
	  logger.debug(" userInfo : " + userInfo); //{name=시스템관리자, userName=admin, type=300003, userRole=4, userId=69}
	  logger.debug(" method : " + method); // GET
	  logger.debug("★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★");
	    
	  switch(method) {
	  case GET :
		  
	        if("300004".equals(userInfo.get("type"))){//에이전트, 브랜드
	        	params.put("franId", String.valueOf(userInfo.get("franId")));
	    		params.put("brandId", String.valueOf(userInfo.get("brandId")));  
	    	}else if("300003".equals(userInfo.get("type"))){//시스템관리자
	  ...
	  case POST : // 라이센스 일괄등록
		  license.setServiceId(Globals.PICO_SERVICE_ID); // Globals.PICO_SERVICE_ID = 1
	      license.setTenantId(Globals.PICO_TENANT_ID); // Globals.PICO_TENANT_ID = 1
	      
	      boolean flag = true;
		  int licenseCnt = StrUtils.parseInt(search.get("licenseCnt")); //라이센스 수량
		  	  
	      if (licenseCnt > 0) {
	    	  for (int i = 0; i < licenseCnt; i++) { //
	    		  license.setLicenseKey(RandomUtil.getLicense()); //RandomUtil 도구클래스를 만들어서 -> java에서 제공하는 UUID 클래스를 import하여 randomUUID() 메서드를 이용하여 라이센스를 만듬
	    		  license.setStatus(codeUtil.getBaseCodeByAlias("poslicense-status-unuse"));
