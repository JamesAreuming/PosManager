...
*/				
	 ...
	 */
	@Override
	public SvcTableExtended saveOrderKiosk(ServletContext context, SingleMap param) throws RequestResolveException {
        
		// data : (1) withTableLock, (2) tableId, (3) posNo, (4)isUserPrinter, (5) Order
		...
		try {
			...
			if (!order.getSvcOrderPays().isEmpty()) {
				...
				order.setPayMethod(order.getSvcOrderPays().get(0).getPayMethod()); //결제 List 첫번째
			}
			
			order.setIsUsePrinter(isUsePrinter); //신용카드 : true, 현금 : false
			order = orderService.saveOrderKiosk(order, host); //order정보
